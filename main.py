...
# v0003 (2026-07-07) - 사무실/정비실 등 긴급연락처 입력가능 및 연도 표시 유지
# ==========================================
...
STORAGE_EMERGENCY_KEY = "bus_helper_emergency" [cite: 134]
...
def main(page: ft.Page):
    ...
    saved_emergency = page.client_storage.get(STORAGE_EMERGENCY_KEY) [cite: 137]
    if saved_emergency:
    ...
    def save_all_to_client_storage():
        ...
        page.client_storage.set(STORAGE_EMERGENCY_KEY, json.dumps(EMERGENCY_LIST, ensure_ascii=False)) [cite: 139]
    ...
    input_zone_container = ft.Column(spacing=2, visible=False) [cite: 140]
    ...
    phonebook_zone_container = ft.Column([
        ft.Container(
            content=ft.Column([
                ...
                ], alignment="spaceBetween"), [cite: 141]
                ft.Divider(height=1),
                ft.Row([
                    ...
                    ft.ElevatedButton( [cite: 142]
                        ...
                        on_click=lambda e: add_phonebook_item() [cite: 143]
                    )
                ], spacing=4),
                ft.Divider(height=1, color="#E2E8F0"),
                phonebook_items_column
            ]),
            padding=12, border=ft.border.all(1, "#2563EB"), border_radius=10 [cite: 144]
        )
    ...
    btn_setting = ft.ElevatedButton(
        content=ft.Container(ft.Text("긴급연락처", color="white", size=11, weight="bold"), alignment=ft.alignment.center), [cite: 146]
        expand=1, height=40, 
        style=ft.ButtonStyle(bgcolor="grey", shape=ft.RoundedRectangleBorder(radius=6), padding=ft.padding.symmetric(vertical=0, horizontal=0)), 
    ...
    def rebuild_phonebook_view():
        phonebook_items_column.controls.clear() [cite: 147]
        if not PHONEBOOK_LIST:
            phonebook_items_column.controls.append(
                ft.Container(
                    ...
                    padding=20, alignment=ft.alignment.center [cite: 148]
                )
            )
        else:
            for index, item in enumerate(PHONEBOOK_LIST):
                ...
                is_edit = item.get("is_edit", False) [cite: 149]
                ...
                if is_edit:
                    ...
                    def save_edit(idx, en, ep):
                        if en.value and ep.value:
                            ...
                            PHONEBOOK_LIST.sort(key=lambda x: x.get("name", "")) [cite: 151]
                    ...
                    row_content = ft.Row([
                        ...
                        ft.ElevatedButton(
                            ...
                            bgcolor="green", width=50, height=34, [cite: 153]
                            style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=4), padding=0),
                        ...
                        ft.ElevatedButton(
                            ...
                            bgcolor="grey", width=50, height=34, [cite: 155]
                            style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=4), padding=0),
                    ...
                    row_content = ft.Row([
                        ft.GestureDetector(
                            content=ft.Row([
                                ft.Text(f"{name}", size=14, weight="bold", color="black", width=65), [cite: 157]
                                ft.Text(f"{phone}", size=13, weight="bold", color="#1E3A8A", no_wrap=True), 
                                ft.Text("☎️", size=11, color="red") [cite: 158]
                            ], spacing=4, alignment="start"),
                            on_tap=lambda e, p=phone: make_call(p),
                            expand=True [cite: 159]
                        ),
                        ft.Row([
                            ft.ElevatedButton(
                                content=ft.Container(ft.Text("수정", size=10, weight="bold", color="white"), alignment=ft.alignment.center), [cite: 160]
                                bgcolor="#2563EB", width=40, height=28,
                                style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=4), padding=0),
                                on_click=lambda e, idx=index: toggle_edit_mode(idx, True) [cite: 161]
                            ),
                            ft.ElevatedButton(
                                content=ft.Container(ft.Text("삭제", size=10, weight="bold", color="white"), alignment=ft.alignment.center), [cite: 162]
                                bgcolor="#1E3A8A", width=40, height=28,
                                style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=4), padding=0),
                                on_click=lambda e, idx=index: delete_phonebook_item(idx) [cite: 163]
                            )
                        ], spacing=3)
                    ], alignment="spaceBetween")

                phonebook_items_column.controls.append(
                    ft.Container( [cite: 164]
                        content=row_content,
    ...
    # 🌟 [빌드 0003 중요 교정] 입력창은 보존하고, 리스트 출력구역(emergency_list_container)만 비우고 다시 그립니다.
    def rebuild_emergency_view(target_column):
        emergency_list_container.controls.clear()
        
        for index, item in enumerate(EMERGENCY_LIST):
            is_fixed = item.get("is_fixed", False)
            display_text = f"{item['name']}: {item['phone']}" if item['phone'] else f"{item['name']}: (번호 없음)" [cite: 166]
            
            action_buttons = [
                ft.IconButton(
                    icon=ft.icons.PHONE,
                    icon_color="green", [cite: 167]
            ...
            if not is_fixed:
                action_buttons.append(
                    ft.ElevatedButton( [cite: 168]
                        content=ft.Container(ft.Text("삭제", size=10, weight="bold", color="white"), alignment=ft.alignment.center),
                        bgcolor="#1E3A8A", width=40, height=28,
                        style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=4), padding=0),
                        on_click=lambda e, idx=index: delete_emergency_item(idx, target_column) [cite: 169]
                    )
                )
            
            row_content = ft.Row([
                ft.Text(display_text, size=14, weight="bold" if is_fixed else "normal", color="black"), [cite: 170]
                ft.Row(action_buttons, spacing=3)
            ], alignment="spaceBetween")
            
            emergency_list_container.controls.append(
                ft.Container(
                    content=row_content, [cite: 171]
                    padding=ft.padding.only(left=4, right=4, top=8, bottom=8),
                    border=ft.border.Border(bottom=ft.border.BorderSide(0.5, "#E2E8F0"))
                )
            )
    ...
    def delete_emergency_item(index, target_column): [cite: 172]
        EMERGENCY_LIST.pop(index)
        save_all_to_client_storage()
        rebuild_emergency_view(target_column)

    # 🌟 [빌드 0003 신설] 버튼 눌렀을 때 제대로 데이터를 저장해 주는 정답 함수 장착!
    def add_emergency_item():
        name_val = em_name.value.strip()   
        phone_val = em_phone.value.strip() 
        
        if not name_val or not phone_val:
            return
            
        formatted_phone = final_format_phone(phone_val)
        
        found = False
        for item in EMERGENCY_LIST:
            if item["name"] == name_val:
                item["phone"] = formatted_phone 
                found = True
                break
                
        if not found:
            EMERGENCY_LIST.append({"name": name_val, "phone": formatted_phone, "is_fixed": False})
    ...
    def add_phonebook_item():
        ...
        if tf_name.value and tf_phone.value:
            ...
            PHONEBOOK_LIST.sort(key=lambda x: x.get("name", "")) [cite: 173]
            save_all_to_client_storage()
            tf_name.value = ""
            tf_phone.value = ""
            rebuild_phonebook_view()
    ...
    def delete_phonebook_item(index):
        if 0 <= index < len(PHONEBOOK_LIST):
            ...
            save_all_to_client_storage()
            rebuild_phonebook_view() [cite: 174]
    ...
    def change_tab(tab_name):
        ...
        btn_calendar.style = ft.ButtonStyle(
            color="white" if tab_name == "달력" else "#94A3B8", [cite: 175]
        ...
        btn_input.style = ft.ButtonStyle(
            ...
            bgcolor="#2563EB" if tab_name == "운행정보" else "transparent", [cite: 176]
            shape=ft.RoundedRectangleBorder(radius=6)
        )
        ...
        if tab_name == "달력":
            ...
            div_line2.visible = True [cite: 179]
            
        elif tab_name == "운행정보":
            header_nav.visible = True [cite: 180]
            ...
            setting_column.visible = False
            weeks_header.visible = False [cite: 183]
            div_line1.visible = False
            div_line2.visible = False
            ...
            header_nav.visible = True [cite: 184]
            summary_area.visible = False
            guide_text.visible = False
            ...
            input_zone_container.visible = False
            phonebook_zone_container.visible = False
            setting_column.visible = True [cite: 185]
    ...
    order_options = [ft.dropdown.Option("", "선택 안 함")] + [ft.dropdown.Option(str(i), f"{i}번") for i in range(1, 51)] [cite: 186]
    ...
    dial_row = ft.Row([hour_picker, ft.Text(":", size=20, weight="bold", color="black"), minute_picker], alignment="center", height=100) [cite: 188]
    ...
    def get_mangeun_target():
        try:
            ...
            days_in_month = calendar.monthrange(y, m)[1] [cite: 189]
    ...
    def build_driving_summary_zone():
        my_card = ft.Container(
            content=ft.Column([ [cite: 190]
                ft.Row([
                    ft.Text("내차 정보", size=11, color="grey", weight="bold"),
                    ft.ElevatedButton(
                        content=ft.Container(ft.Text("입력", size=10, weight="bold", color="white"), alignment=ft.alignment.center), [cite: 191]
                ...
                ft.Text(f"노선: {input_data_state['route']}", size=14, weight="bold", color="black"), [cite: 192]
        ...
        front_card = ft.Container(
            content=ft.Column([ [cite: 193]
                ft.Row([
                    ft.Text("앞차 정보", size=11, color="grey", weight="bold"),
                    ft.ElevatedButton(
                        content=ft.Container(ft.Text("입력", size=10, weight="bold", color="white"), alignment=ft.alignment.center), [cite: 194]
                        on_click=lambda e: open_info_input_popup("앞차"), bgcolor="#1E3A8A", width=55, height=22, 
                        style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=4), padding=ft.padding.symmetric(vertical=0, horizontal=0))
                    )
                ], alignment="spaceBetween"),
                ft.Text(input_data_state['front_bus'], size=14, weight="bold", color="black"), [cite: 195]
                ...
                ft.GestureDetector(
                    content=ft.Row([
                        ...
                        ft.Text("☎️", size=13, color="red") if input_data_state['front_phone'] != "미입력" else ft.Container() [cite: 196]
                    ], spacing=4, alignment="start"),
                    on_tap=lambda e: make_call(input_data_state['front_phone'])
                )
            ], spacing=2, tight=True),
            bgcolor="#F8FAFC", border=ft.border.all(1, "#E2E8F0"), border_radius=8, padding=10, expand=1 [cite: 197]
        )
        ...
        back_card = ft.Container(
            content=ft.Column([
                ft.Row([
                    ...
                    ft.ElevatedButton(
                        content=ft.Container(ft.Text("입력", size=10, weight="bold", color="white"), alignment=ft.alignment.center), [cite: 198]
                ...
                ft.GestureDetector(
                    content=ft.Row([
                        ft.Text(input_data_state['back_phone'], size=13, color="#1E3A8A", weight="bold"), [cite: 200]
        ...
        return ft.Container(
            content=ft.Column([
                ft.Text("🚍 운행 정보 요약", size=14, weight="bold", color="#1E3A8A"), [cite: 202]
                my_card, 
    ...
    def open_info_input_popup(target_type):
        if target_type == "내차":
            tf_route = ft.TextField(label="노선번호", value=input_data_state["route"].replace("미입력",""), keyboard_type=ft.KeyboardType.NUMBER, expand=True, height=38) [cite: 204]
            ...
            def save_my(e):
                ...
                input_data_state["bus_no"] = f"{tf_bus_no.value}호" if tf_bus_no.value else "미입력" [cite: 205]
                save_all_to_client_storage()
            ...
            box_content = ft.Container(
                content=ft.Column([ [cite: 206]
                    ft.Text("👤 내 차량 설정", size=14, weight="bold"),
                    ft.Row([tf_route, tf_bus_no]),
                    ft.Row([
                        ft.ElevatedButton(
                            content=ft.Container(ft.Text("확인", size=13, weight="bold", color="white"), alignment=ft.alignment.center), [cite: 207]
                        ...
                        ft.ElevatedButton(
                            ...
                            on_click=lambda e: setattr(info_dialog, "open", False) or page.update(), expand=1, height=38, [cite: 209]
            ...
            def save_front(e):
                ...
                save_all_to_client_storage()
                info_dialog.open = False  [cite: 212]
                page.update()
                rebuild_interface()

            box_content = ft.Container(
                content=ft.Column([
                    ft.Text("◀ 앞차 정보 입력", size=14, weight="bold"), [cite: 213]
                    tf_f_bus, tf_f_driver, tf_f_phone,
                    ft.Row([
                        ft.ElevatedButton(
                            content=ft.Container(ft.Text("확인", size=13, weight="bold", color="white"), alignment=ft.alignment.center), [cite: 214]
                        ...
                        ft.ElevatedButton( [cite: 215]
                            content=ft.Container(ft.Text("뒤로가기", size=13, weight="bold", color="white"), alignment=ft.alignment.center),
                            on_click=lambda e: setattr(info_dialog, "open", False) or page.update(), expand=1, height=38,
                            style=ft.ButtonStyle(bgcolor="grey", shape=ft.RoundedRectangleBorder(radius=0), padding=ft.padding.symmetric(vertical=0, horizontal=0)), [cite: 216]
                        )
                    ], alignment="center", spacing=8)
                ], spacing=10, tight=True),
                width=260, padding=4
            ...
            def save_back(e):
                input_data_state["back_bus"] = f"{tf_b_bus.value}호" if tf_b_bus.value else "미입력" [cite: 218]
                ...
                save_all_to_client_storage()
                info_dialog.open = False  
                page.update() [cite: 219]
                rebuild_interface()

            box_content = ft.Container(
                content=ft.Column([
                    ft.Text("▶ 뒷차 정보 입력", size=14, weight="bold"),
                    tf_b_bus, tf_b_driver, tf_b_phone, [cite: 220]
                    ft.Row([
                        ft.ElevatedButton(
                            content=ft.Container(ft.Text("확인", size=13, weight="bold", color="white"), alignment=ft.alignment.center),
                            on_click=save_back, expand=1, height=38, [cite: 221]
                            style=ft.ButtonStyle(bgcolor="#1E3A8A", shape=ft.RoundedRectangleBorder(radius=0), padding=ft.padding.symmetric(vertical=0, horizontal=0)),
                        ),
                        ft.ElevatedButton(
                            content=ft.Container(ft.Text("뒤로가기", size=13, weight="bold", color="white"), alignment=ft.alignment.center), [cite: 222]
                            on_click=lambda e: setattr(info_dialog, "open", False) or page.update(), expand=1, height=38,
                            style=ft.ButtonStyle(bgcolor="grey", shape=ft.RoundedRectangleBorder(radius=0), padding=ft.padding.symmetric(vertical=0, horizontal=0)),
                        ) [cite: 223]
                    ], alignment="center", spacing=8)
    ...
    def rebuild_interface():
        ...
        month_prefix = f"{current['year']}-{current['month']:02d}" [cite: 225]
        ...
        work_summary = f"근무: {work_days}(+{diff})" if diff > 0 else (f"근무: {work_days}({diff})" if diff < 0 else f"근무: {work_days}") [cite: 226]
        ...
        for week in month_weeks:
            week_row = ft.Row(alignment="spaceAround", spacing=2) [cite: 227]
            for day in week:
                if day == 0:
                    week_row.controls.append(ft.Container(expand=1, height=52))
                else:
                    weekday = datetime(current['year'], current['month'], day).weekday() [cite: 228]
                    ...
                    is_today = (current['year'] == today_y and current['month'] == today_m and day == today_d) [cite: 233]
                    day_border = ft.border.all(2, "#2563EB") if is_today else ft.border.all(0.5, "#E2E8F0")

                    day_box = ft.Container(
                        content=ft.Column([ft.Text(f"{day}", size=12, weight="bold", color=day_number_color), ft.Text(status_desc, size=10, weight="bold", color=text_color), time_display], alignment="center", horizontal_alignment="center", spacing=0), [cite: 234]
    ...
    def open_input_popup(date_key):
        ...
        order_dropdown.value = str(current_order) if current_order else "" [cite: 236]
        if current_time and ":" in current_time:
            ...
            hour_picker.selected_index, minute_picker.selected_index = 5, 0 [cite: 237]
    ...
    def select_status_and_save(status_value):
        ...
        if status_value == "선택취소":
            ...
            save_all_to_client_storage()
            popup_layer.visible = False [cite: 238]
            rebuild_interface()
            return
        ...
        if status_value == "자동":
            ...
            if existing_info.get("status") == "전일": [cite: 239]
                status_value = "전일"
            else:
    ...
    popup_card = ft.Container(
        content=ft.Column([
            ...
            ft.Divider(height=1, color="transparent"), [cite: 241]
            ...
            ft.Row([
                ...
                ft.Container(content=ft.Text("오후", size=14, weight="bold", color="white"), bgcolor="#7E22CE", alignment=ft.Alignment(0, 0), height=38, border_radius=6, expand=1, on_click=lambda e: select_status_and_save("오후")), [cite: 242]
    ...
    def open_mangeun_popup(e):
        mangeun_dropdown.value = str(get_mangeun_target()) [cite: 244]
    ...
    mangeun_popup_card = ft.Container(
        ...
        ], spacing=10, tight=True), [cite: 245]
    ...
    def move_prev(e):
        current["month"] -= 1
        if current["month"] == 0: current["month"] = 12; current["year"] -= 1 [cite: 246]
        rebuild_interface()

    def move_next(e):
        current["month"] += 1
        if current["month"] == 13: current["month"] = 1; current["year"] += 1 [cite: 247]
        rebuild_interface()
    ...
    # 🌟 [임시 리스트 출력상자 분리] 새로고침해도 입력창이 안 지워지도록 담아두는 전용 통입니다.
    emergency_list_container = ft.Column(spacing=6)

    # 🌟 [디자인 틀 완벽 통합] 입력창과 출력 리스트 박스를 한 세트로 묶었습니다. [cite: 248]
    setting_column = ft.Column([
        ft.Container(
            content=ft.Column([
                ...
                ft.Divider(height=1), [cite: 249]
                
                ft.Row([
                    em_name := ft.TextField(label="이름/서비스명", label_style=ft.TextStyle(size=11), width=100, height=38, text_size=13, content_padding=8),
                    em_phone := ft.TextField(label="전화번호(숫자만)", label_style=ft.TextStyle(size=11), expand=True, height=38, text_size=13, content_padding=8, keyboard_type=ft.KeyboardType.PHONE), [cite: 250]
                    ft.ElevatedButton(
                        content=ft.Text("등록", size=12, weight="bold", color="white"),
                        bgcolor="#2563EB", width=60, height=38,
                        style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=4), padding=0), [cite: 251]
                        on_click=lambda e: add_emergency_item() 
                    )
                ], spacing=4),
                ft.Divider(height=1, color="#E2E8F0"), [cite: 252]
                
                # 여기에만 주르륵 목록이 새로고침되어 추가됩니다!
                emergency_list_container
            ]), [cite: 253]
